...
export default async function handler(req, res) {
    ...
    try {
        ...
        const { count: orderCount } = await supabase
            .from('gg_orders').select('*', { count: 'exact', head: true }).eq('user_id', user.id);
        ...
        // 2. CEK BUKU CATATAN VOUCHER (Apa saja yang sudah dipakai user ini?)
        ...
        const filteredVouchers = dbVouchers.filter(vch => {
            // JIKA USER SUDAH PERNAH CLAIM KODE INI, SEMBUNYIKAN!
